<template>
  <div class="edit-subproduct">
    <ul class="form">
      <h2 class="form-title">Chỉnh sửa sản phẩm con</h2>
      <li class="field">
        <p>Mã code</p>
      ...
      <li class="field pick-img">
        <div class="img-title">
          <p>Hỉnh ảnh</p>
          <button class="btnAdd" @click="onChooseFile">
      ...
      <li class="field">
        ...
        <ul class="list-info">
          <li class="info" v-for="(info, ind) in newData.infor" :key="ind">
            <input
              type="text"
              placeholder="Nhập nội dung giới thiệu"
              v-model="newData.infor[ind]"
            />
      ...
      <li class="form-action">
        <button class="btn-cancel" @click="cancelEditSub">Hủy</button>
...
export default {
  name: "EditSubProduct",
  async beforeMount() {
    this.newData = { ...this.currentSubProduct };
    this.newData.price = {
      origin: this.newData.origin_price,
      final: this.newData.final_price,
    };
    delete this.newData.origin_price;
    delete this.newData.final_price;
  ...
  methods: {
    async previewImg() {
      if (this.newImgUrl == null) {
        this.imgUrl = await firebaseUtils.getFriebaseFileUrl(this.newData.src);
      }
    },
    onChooseFile() {
    ...
    addInfo() {
      this.newData.infor.push("");
    },
    removeInfo(index) {
      this.newData.infor.splice(index, 1);
    },
    cancelEditSub() {
      if (confirm("Bạn chắc chắn muốn hủy?")) {
        this.$emit("onCancelEditSubProduct");
      }
    },
    async onFinish() {
    ...
    async editSub() {
      ...
      await this.putEditSub();
    },

    async uploadSubImg() {
      if (this.isChangeImg) {
        let src = await firebaseUtils.uploadFileToFirebase(this.imgFile);
        if (src != null) {
    ...
    async putEditSub() {
      await axios
        ...
        .then((res) => {
          ...
          this.$emit("onFinishEditSubProduct");
        })
        .catch((e) => {
          console.log(e);
        });
